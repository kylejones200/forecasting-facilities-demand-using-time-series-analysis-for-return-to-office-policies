...
def main():
    ...
    for emp_id in range(1, n_employees + 1):
        for day in range(1, n_days + 1):
            if np.random.rand() < 0.6:  # 60% chance the employee comes to the office
                ...
                badge_out_time = badge_in_time + np.random.normal(
                    loc=8, scale=1.5
                )  # 8-hour shift
                badge_data.append(
                    [
                        emp_id,
                        day,
                        max(7, min(11, badge_in_time)),
                        max(15, min(20, badge_out_time)),
                    ]
                )
    # Convert to DataFrame
    df_badge = pd.DataFrame(
        badge_data, columns=["Employee_ID", "Day", "Badge_In", "Badge_Out"]
    )
    ...
    plt.plot(
        daily_badges.index, daily_badges.values, color="black", label="Employees Badged In"
    )
    plt.xlabel("Day")
    ...
    plt.figure(figsize=(12, 6))
    plt.plot(
        daily_badges.index, daily_badges.values, label="Actual Attendance", color="black"
    )
    plt.plot(
    ...
    df_badge.loc[df_badge["Employee_ID"].isin(coffee_badger_ids), "Badge_Out"] = (
        df_badge.loc[df_badge["Employee_ID"].isin(coffee_badger_ids), "Badge_In"]
        + np.random.uniform(
